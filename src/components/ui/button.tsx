  ...
  "inline-flex items-center justify-center rounded-sm text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        destructive:
          "bg-destructive/10 border-destructive-foreground border text-destructive-foreground hover:bg-destructive/90",
        outline:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, ...props }, ref) => {
    ...
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, className }), loading && "opacity-50 flex items-center justify-center gap-2")}
        ref={ref}
        {...props}
      >
        <Loader2 className={cn("hidden", loading && "block animate-spin")} />
        {props.children}
      </Comp>
    )
  }
