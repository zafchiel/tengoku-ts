...
export default async function AllCharactersPage({ params: { animeId } }: AllCharactersPageProps) {
    ...
    return (
        <main className="container space-y-8 px-2 md:px-4 md:py-14 md:mt-10">
            <h2 className="text-3xl md:text-5xl"><span
                className="text-muted-foreground">characters: </span>{animeInfo.title}</h2>

            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm md:text-base">
                {characters.map((character) => (
                    <CharacterCardFull key={character.character.mal_id} character={character}/>
                ))}
            </section>
        </main>
    );
